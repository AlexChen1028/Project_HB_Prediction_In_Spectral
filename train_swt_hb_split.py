...
def load_dataset(base_dir, mua_folder):
    ...
    for f_name in tqdm(files, desc="SWT 特徵提取中"):
        ...
        if row.empty or pd.isnull(row.iloc[0]['ClinicHb']):
            continue

        feats = _extract_features(os.path.join(mua_folder, f_name))
    ...
    print(f"\n>>> 配對成功: {len(labels)} 筆樣本 / {n_pat} 位病人")
    return raw_features, labels, patient_ids
...
def train_group(raw_features, labels, patient_ids, group_name, model_save_path, tag):
    ...
    print(f"\n{'='*60}")
    print(f"  群組: {group_name}  |  樣本數: {n_samples}")
    print(f"{'='*60}")
    ...
    if len(tr_idx) == 0 or len(test_idx) == 0:
        print(f"  [{group_name}] Train 或 Test 樣本不足，跳過")
        return

    tr_raw = raw_features[tr_idx]
    tr_labels = labels[tr_idx]
    tr_pids_local = [patient_ids[i] for i in tr_idx]
    unique_tr_patients = np.unique(tr_pids_local)

    # K-Fold on train pool
    ...
    for fold, (f_tr_pat_idx, f_val_pat_idx) in enumerate(kf.split(unique_tr_patients)):
        f_tr_patients = set(unique_tr_patients[f_tr_pat_idx])
        f_val_patients = set(unique_tr_patients[f_val_pat_idx])

        f_tr_local = [i for i, pid in enumerate(tr_pids_local) if pid in f_tr_patients]
        f_val_local = [i for i, pid in enumerate(tr_pids_local) if pid in f_val_patients]

        if len(f_tr_local) < BATCH_SIZE or len(f_val_local) < 2:
            print(f"  Fold {fold+1}: 樣本不足，跳過")
            continue

        f_tr_raw = tr_raw[f_tr_local]
        f_val_raw = tr_raw[f_val_local]
        # 三個 channel 共用同一 mean/std，保留彼此相對關係
        ...
        with torch.no_grad():
            for x, y in f_val_loader:
                ...
                y_pred_f.extend(pred.cpu().numpy())
    ...
    # Final model — train on full train pool, monitor on test set
    print(f"\n  最終模型訓練 ({group_name})")
    ...
    for epoch in pbar:
        ...
        if avg_te < best_test_loss:
            ...
            torch.save({
                ...
                'group_is_low': labels.min() < HB_THRESHOLD and labels.max() < HB_THRESHOLD
                               or tag == 'low',
            }, model_save_path)
...
def train():
    ...
    print(f"\n>>> train : test = {TRAIN_RATIO} : 1  (test = n_patients // {TRAIN_RATIO+1})")
