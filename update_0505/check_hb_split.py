...
for path in glob.glob(os.path.join(BASE_DIR, '2026*_dialysis_table_export.xlsx')):
    ...
    if key:
        excel_cache[key.group(1)] = df

if not excel_cache:
    print(f'找不到 Excel，請確認路徑: {BASE_DIR}')
    exit()

# ── 掃描光譜檔，配對 Excel ─────────────────────────────
if not os.path.isdir(MUA_FOLDER):
    print(f'找不到光譜資料夾: {MUA_FOLDER}')
    exit()

records = []   # list of dict: {fname, date, pid, hb}
...
summary(grp_high, f'HB >= {HB_THRESHOLD}')
...
print()
print('  未配對光譜（無對應 Excel 或無 ClinicHb）:',
      len(txt_files) - len(df_all), '個')
