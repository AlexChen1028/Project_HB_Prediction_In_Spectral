...
def _extract_features(mua_path):
    ...
    if len(v) < 896:
        ...
        v = v[:896]
    coeffs = pywt.swt(v, wavelet='db4', level=3)
    cA3 = coeffs[0][0]

    def pt(idx):
        return np.mean(cA3[max(0, idx - WINDOW_W): idx + WINDOW_W + 1])

    v540 = pt(IDX_540)
    v560 = pt(IDX_560)
    v577 = pt(IDX_577)

    return np.array([v540, v560, v577], dtype=np.float32)
...
def load_dataset(base_dir, mua_folder):
    ...
    return np.array(raw_features, dtype=np.float32), np.array(labels, dtype=np.float32), patient_ids
...
def draw_4panel(y_true, y_pred, group_name, fig_path):
    ...
    ax1 = axes[0, 0]
    lim_lo = min(y_true.min(), y_pred.min()) - 0.5
    lim_hi = max(y_true.max(), y_pred.max()) + 0.5
    ax1.scatter(y_true, y_pred, alpha=0.6, color='dodgerblue', edgecolors='none')
    ax1.plot([lim_lo, lim_hi], [lim_lo, lim_hi], 'r--', lw=2, label='Perfect fit')
    ax1.set_xlim(lim_lo, lim_hi)
    ax1.set_ylim(lim_lo, lim_hi)
    ax1.set_xlabel('Actual ClinicHb (g/dL)')
    ax1.set_ylabel('Predicted ClinicHb (g/dL)')
    ax1.set_title(f'Regression Plot  R²={r2:.3f}  MAE={mae:.3f}')
    ax1.legend()
    ax1.grid(True, linestyle=':', alpha=0.6)

    # 2. Sorted prediction (ascending by actual Hb)
    ax2 = axes[0, 1]
    sort_idx = np.argsort(y_true)
    ax2.plot(y_true[sort_idx], label='Actual', color='black',
             marker='o', markersize=3, linewidth=1, alpha=0.7)
    ax2.plot(y_pred[sort_idx], label='Predicted', color='red',
             marker='x', markersize=3, linewidth=1, alpha=0.7)
    ax2.axhline(y=HB_THRESHOLD, color='green', linestyle='--',
                alpha=0.7, label=f'Threshold ({HB_THRESHOLD})')
    ax2.set_xlabel('Sample index (sorted by actual Hb, low → high)')
    ax2.set_ylabel('ClinicHb (g/dL)')
    ax2.set_title('Sorted Prediction')
    ...
    ax3.axhline(y=1,  color='gray', linestyle=':', lw=1, alpha=0.7)
    ax3.axhline(y=-1, color='gray', linestyle=':', lw=1, alpha=0.7)
    ax3.set_xlabel('Predicted ClinicHb (g/dL)')
    ax3.set_ylabel('Residual  (Actual − Predicted)')
    ax3.set_title('Residual Plot')
    ax3.grid(True, linestyle=':', alpha=0.6)

    # 4. Normal Probability Plot (PP Plot) of residuals
    ax4 = axes[1, 1]
    ...
    ax4.set_ylabel('Sample Quantiles  (Residuals)')
    ax4.set_title(f'Normal Probability Plot of Residuals  (R={r_val:.3f})')
...
def eval_group(model_path, raw_features_all, labels_all, patient_ids_all,
    ...
    if not os.path.exists(model_path):
        print(f"  找不到模型，請先執行 train_swt_hb_split.py")
        return
    ...
    model = HbRawNet().to(DEVICE)
    model.load_state_dict(ckpt['model'])
    model.eval()

    # Filter: test patients AND correct HB group
    if is_low:
        group_mask = labels_all < HB_THRESHOLD
    else:
        group_mask = labels_all >= HB_THRESHOLD
    ...
    print(f"  [{group_name}] Test set: {len(labels)} 筆樣本 / {n_pats} 位病人")

    if len(labels) == 0:
        print(f"  [{group_name}] 無測試樣本，跳過")
        return
    ...
    with torch.no_grad():
        for i in range(0, len(X_tensor), 32):
            batch = X_tensor[i:i + 32].to(DEVICE)
            y_pred.extend(model(batch).cpu().numpy().tolist())
    ...
    base_mae = mean_absolute_error(y_true, np.full_like(y_true, y_mean))
...
def evaluate():
    ...
    eval_group(
        LOW_MODEL_PATH, raw_features, labels, patient_ids,
        group_name=f'HB < {HB_THRESHOLD}',
        is_low=True,
        fig_tag='low'
    )

    eval_group(
        HIGH_MODEL_PATH, raw_features, labels, patient_ids,
        group_name=f'HB >= {HB_THRESHOLD}',
        is_low=False,
        fig_tag='high'
    )
